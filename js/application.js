...
//定义分享js
//此js分享内容会优先加载  页面中重定义的分享内容会覆盖
if(typeof WeChatShare!="undefined"){
    var sendShareData={};
    //登录时分享 redirect不定义 跳转至获客页  未登录分享 定义redirect 跳转至首页
    if(window.loginInfo&&!window.loginInfo.isLogin){
        sendShareData.redirect=location.origin
    }
    var wechatShare=new WeChatShare();
    $.get("/secure/referrer-url.html",sendShareData, function(data){
        var shareData ={
            shareConfig:{
                title:data.data.share.title,
                desc: data.data.share.description,
                link: data.data.url,
                imgUrl: "http://bxwd-img.oss-cn-hangzhou.aliyuncs.com/"+data.data.share.coverUrl
            },
            menuList:{
                menuList: [
