(function () {
    ...
    var delay = getQueryString('delay') ? getQueryString('delay') : 7,URI= 'https://g.bxwd.cc/';
var placeList = [
...
var placeList2 = [
    {name:'上海市', geoCoord:[121.48, 31.22]},
    {name:'上海市', geoCoord:[121.480656, 31.22]},
    {name:'上海市', geoCoord:[121.48, 31.23375]},
    {name:'北京市', geoCoord:[116.46, 39.92]},
    {name:'廊坊市', geoCoord:[116.7, 39.53]},
    {name:'菏泽市', geoCoord:[115.480656, 35.23375]},
    {name:'济南市', geoCoord:[117, 36.65]},
...
'株洲市': [113.5327,27.0319],'淄博市': [118.0371,36.6064]
        ...
        $.ajax({
          ...
          , error: function (){
            $.ajax({
              type: "POST",
              data : {username:'relitu',password:'111111'},
              url: "https://g.bxwd.cc/login/login"
            });
            $.ajax({
              type: "POST",
              data : {username:'relitu',password:'111111'},
              url: "https://boss.bxjr.cc/login"
            });
        ...
        /**/
        var t = '<li>位于 {b} 的宝象会员 将 <em>&yen;{c} </em>元 投资到 {d}</li>';
        function updateText(){
    ...
    (function(){
        ...
        function toUpData(){
            $.post('/wd_api/about/getNoticeDetailByTypeOn', {api_key:0,bt:2,code:'usermap_news',ct:1,typeCode:'ddd',sign:'B511E0915B500D87F7A16BA07C686D4E'}, function(json){
                var list = json.data.descriptionDetail.split('\r\n');

                var strStoreDate = localStorage.getItem("news");

                if(!strStoreDate){
                    localStorage.setItem("news", list);
                }

                var news = $(".newsCont");

                if(news.html().length < 12 || (strStoreDate != list.toString())){
                    news.html(list.toString().replace(/,/g, "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"));
                }
            },'json');
        }
        ...
        function tohtml(_num){
            ...
            if($('#today_num .num_visit').length == 0 || $('#today_num .num_visit').length != num.length){
                $('#today_num').html(createDemo(num.length));
            }

            for(var i = 0, len = arr.length; i < len; i ++){
                var position = -(parseInt(arr[i])) * 92;
                $("#today_num .num_visit").eq(i).animate({marginTop: position});
            }
        }

        function createDemo(len){
            var html = "";
            for(var i = 0; i < len; i ++){
        ...
        function updatehtml(_num){
            var arr = [],
                num = _num.toFixed(2).toString();

            num = num.substr(0, num.length - 3);
            arr = num.replace(/(.)(?=[^$])/g,"$1,").split(",");

            if($('.invest .num_visit').length == 0 || $('.invest .num_visit').length != num.length){
                $('.invest .cont .number').html(createDemo(num.length));
            }

            for(var i = 0, len = arr.length; i < len; i ++){
                var position = -(parseInt(arr[i])) * 92;
                $('.invest .num_visit').eq(i).animate({marginTop: position});
            }
        }
        ...
        setInterval(function(){
            $.ajax({
                ...
                success: function (json) {
                    tohtml(json.members);
                    updatehtml(json.amount);
					
                    $(".visit p span").text(json.loginPercent);
                    $(".invest p span").text(json.amounmtPercent);
                }
				, error: function (){
					$.ajax({
						type: "POST",
...
$(function(){
    function todayYmd(){
        ...
        var todayYmd = "<i>" + y + "</i>年<i>" + mon + "</i>月<i>" + d + "</i>日";
    ...
    setInterval(function(){
        ...
        var todayHms = "<i>" + h + ":" + m + ":" + s +"</i>";

        $("#time .s2").html(todayHms);
    }, 1000);

    $("#time .s1").html(todayYmd());
