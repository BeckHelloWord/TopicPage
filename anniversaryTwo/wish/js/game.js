(function ($) {
	$.fn.GameA = function (option) {
		...
		<div id="play_name"><input type="text" placeholder="4位  出生月/日" maxlength="4" pattern="([0-9]|[0-9]|[0-9])" /><p>如8月8日，输入0808</p></div>\
				...
				if (TGconfig["play"] === false) {
					...
					$.ajax({
						...
						success: function(json){
                            if(!json.success){
								window.location.href = "/login/index.html?redirect=" + location.href;

								return false;
							}
                            
                            if(!json.enoughTicket){
                                showmsg();
                                $("#msg .cont").eq(1).show().siblings().hide();
                            ...
                            if(json.name == "谢谢惠顾"){
                                $("#msg .cont").eq(0).find("h4").hide();
                            }else{
                                $("#msg .cont").eq(0).find("h4").show();
                            }

							$("#msg .cont").eq(0).show().siblings().hide();
                            $("#msg .cont").eq(0).find("img").attr("src", json.logo);
                            $("#msg .cont").eq(0).find("p").text(json.name);
						}
					});
				}
			},
		...
		$(TGamedom['start']).click(function() {
            ...
            day = val.slice(2, 4);
            date = "2000/" + val.slice(0, 2) + "/" + day;

			if(val.length < 4 || isNaN(val) || (new Date(date).getDate() == +day)){
				layer.open({
                    content: "亲，请按照正确的格式输入哦！",
                    skin: "msg",
                    time: 2
                });
