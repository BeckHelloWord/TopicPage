...
(function ($) {
    $.fn.slides = function (option) {
        ...
        if(option.hoverPause){
            option.schedule(option.progressBar,option.finish,option.play,option.loadWidth);
        }
        ...
        return this.each(function (i) {
            ...
            if (option.hoverPause && option.play) {
                ...
                control.children().bind('mouseleave', function () {
                    var timer=null;
                    if(i===0){
                        option.remainSpace= $(option.progressBar).css("left").replace(/(-|px)+/g,"");    //剩下的距离
                        option.remainTime=parseInt(option.remainSpace/(option.loadWidth/option.play));    //根据默认平均速度计算剩余距离所需时间
                        //利于平均时间跑完剩余的进度，回调animate，利用surplus标记不执行进度条，直接切换下一张
                        //修改surplus标记，执行计时器，在计时器中默认再跑一遍。
                        $(option.progressBar).animate({"left":option.finish},option.remainTime,function(){
                            $(option.progressBar).css("left",-option.loadWidth);
                            option.surplus=true;
                            animate('next', effect);
                            option.surplus=false;
                            pause();
                        });
                    }else{
                        //当不是主Banner时，执行
            ...
            function pause() {
                //当开启鼠标停止模式
                if(option.hoverPause){
                    if(!Boolean(option.pause)){
                        clearTimeout(elem.data('pause'));
                        clearInterval(elem.data('interval'));

                        option.schedule(option.progressBar,option.finish,option.play,option.loadWidth); //当鼠标离开后默认先执行一遍与计时器同步
    ...
    $.fn.slides.option = {
        ...
        bigTarget: false,
        progressBar:"", //进度条标签
        flag:false, //标记是否自动或鼠标进入
        loadWidth:0,    //进度条总长度
        remainSpace:0,  //剩下的距离
        surplus:false,  //剩余距离标记
        finish:"0px",   //终点位置
        remainTime:0,   //剩下距离移动所需时间
        animationStart: function () {
        },
        animationComplete: function () {
        },
        schedule:function(progressBar,finish,play,loadWidth){
            $(progressBar).animate({"left":finish},play,function(){
                $(progressBar).css("left",-loadWidth);
            });
        }   //执行进度条函数
    };
